    ...
    const std::span<T, LSize + RSize> result) noexcept
{
    ...
    {
        grow_expansion(
            std::span<const T, RSize>(result.subspan(i, RSize)),
            sign * rhs[i],
            std::span<T, RSize + 1>(result.subspan(i, RSize + 1)));
    }
}
...
{
    constexpr auto split_point = std::numeric_limits<T>::digits / 2;
